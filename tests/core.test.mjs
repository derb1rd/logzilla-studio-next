...
test("summaryOf: обогащение HTTP-контекстом без дублей", () => {
  ...
  // запись без текстового поля: контекст уже в k=v, без изменений
  assert.equal(LZ.summaryOf({ method: "GET", url: "/x" }), LZ.msgOf({ method: "GET", url: "/x" }));
});
...
test("sqlSnippetOf / summaryOf: компактный SQL-кусочек запроса", () => {
  ...
  // без текстового поля sql не дублируем (msgOf уже развернул k=v)
  assert.ok(!/sql:/.test(LZ.summaryOf({ sql: "select 1" })));
});
